...
(defn create-dataset
  ...
  (let [token   (login-token username password)
        doi     "ds004056"
        version "1.0.2"
        dataset {:doi doi :version version :username username}]
    dataset
    ))
...
(comment
  ...
  (router {:request-method :post
           :uri            "/api/dataset/openneuro"
           :oauth-token    token
           :body-params    {:source "openneuro" :doi "ds004056" :version "1.0.2"}})
...
(comment
  ...
  (async/put! (first channel) {:doi "ds004056" :version "1.0.2"})
  ...
  (def in (async/chan))
  (def out (async/chan))
  (async/pipeline-blocking 1 out (comp z y x w) in)
